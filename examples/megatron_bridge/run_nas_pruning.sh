...
NUM_GPUS=8
# Target MLA per-head dimension. DeepSeek-V4-Flash ships with head_dim=512
# which is disproportionately wide vs a 4B-scale hidden_size=2048. Pass
# e.g. 128 or 256 to shrink wq_b / wkv / wo_a / compressor per-head in the
# HF export step. Set to 0 (or leave blank) to keep the original 512.
# Use conditional default so HEAD_DIM=256 bash run_nas_pruning.sh works.
...
/opt/venv/bin/python3 -m torch.distributed.run --nproc_per_node=$NUM_GPUS prune_deepseek_v4_nas.py \
    ...
    $([ -n "$HEAD_DIM" ] && [ "$HEAD_DIM" -gt 0 ] && echo "--head_dim $HEAD_DIM") \
    --trust_remote_code
...
# head_dim may be null (user didn't pass --head_dim to NAS) — preserve that.
BEST_HEAD_DIM=$(jq -r '.head_dim // empty' "$BEST_CONFIG")
...
echo "  head_dim: ${BEST_HEAD_DIM:-<keep original 512>}"
...
/opt/venv/bin/python3 -m torch.distributed.run --nproc_per_node=$NUM_GPUS prune_deepseek_v4.py \
    ...
    $([ -n "$BEST_HEAD_DIM" ] && echo "--head_dim $BEST_HEAD_DIM") \
    --calibration_samples 1024 \
    --seq_length 2048 \
    --trust_remote_code
