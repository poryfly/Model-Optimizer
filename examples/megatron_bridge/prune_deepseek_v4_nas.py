...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Target MLA per-head dimension for all NAS candidates. The "
            "original DeepSeek-V4-Flash uses head_dim=512; for 4B-scale "
            "targets this is disproportionately wide vs hidden_size. Pass "
            "e.g. 128 or 256 to have NAS factor the MLA attention weight "
            "savings into the parameter budget and let the HF converter "
            "slice wq_b / wkv / wo_a / compressor per-head."
        ),
    )
...
def compute_model_params(hidden_size, num_layers, num_moe_experts, moe_ffn_hidden_size,
                         ...
                         head_dim=None, num_attention_heads=64, o_groups=8,
                         q_lora_rank=1024, o_lora_rank=1024):
    """Compute total parameter count for a given configuration.

    Includes MLA attention projections (wq_a, wq_b, wkv, kv_norm, wo_a, wo_b,
    q_norm) whose size depends on head_dim. MoE dominates for DeepSeek-V4-Flash
    but at aggressive reductions (hidden=2048, head_dim=128) the attention
    weight budget is non-trivial (~50M → ~13M params per layer for the q_b /
    wo_a pair alone).
    """
    # Attention projections (per layer, MLA with grouped output LoRA):
    #   wq_a: [hidden, q_lora_rank]
    #   q_norm: [q_lora_rank]
    #   wq_b: [n_heads * head_dim, q_lora_rank]
    #   wkv:  [head_dim, hidden]
    #   kv_norm: [head_dim]
    #   wo_a: [n_heads * head_dim // o_groups, o_groups * o_lora_rank]
    #   wo_b: [o_groups * o_lora_rank, hidden]
    hd = head_dim if head_dim else 512
    n_heads = num_attention_heads
    attn_params = num_layers * (
        hidden_size * q_lora_rank                                  # wq_a
        + q_lora_rank                                              # q_norm
        + (n_heads * hd) * q_lora_rank                             # wq_b
        + hd * hidden_size                                         # wkv
        + hd                                                       # kv_norm
        + (n_heads * hd // o_groups) * (o_groups * o_lora_rank)    # wo_a
        + (o_groups * o_lora_rank) * hidden_size                   # wo_b
    )
...
def generate_candidates(orig_config, target_ratio, num_candidates, max_reductions,
    ...
    orig_head_dim = getattr(orig_config, 'head_dim', 512)
    orig_n_heads = getattr(orig_config, 'num_attention_heads',
                           getattr(orig_config, 'num_hidden_layers_heads', 64))
    orig_o_groups = getattr(orig_config, 'o_groups', 8)
    orig_q_lora = getattr(orig_config, 'q_lora_rank', 1024)
    orig_o_lora = getattr(orig_config, 'o_lora_rank', 1024)

    hd = target_head_dim if target_head_dim else orig_head_dim
    ...
    orig_params = compute_model_params(
        ...
        head_dim=orig_head_dim, num_attention_heads=orig_n_heads,
        o_groups=orig_o_groups, q_lora_rank=orig_q_lora, o_lora_rank=orig_o_lora,
    )
    ...
    print_rank_0(f"Head dim: original={orig_head_dim}, target={hd}")
    ...
    for h, f, l, e in product(hidden_sizes, ffn_sizes, layer_counts, expert_counts):
        ...
        params = compute_model_params(
            h, l, e, f, shared,
            head_dim=hd, num_attention_heads=orig_n_heads,
            o_groups=orig_o_groups, q_lora_rank=orig_q_lora, o_lora_rank=orig_o_lora,
        )
        ...
        if abs(ratio - target_ratio) <= ratio_tol:
            all_candidates.append({
                ...
                'head_dim': hd if target_head_dim else None,
                'params_ratio': ratio,
            })
    ...
    for i, c in enumerate(candidates):
        print_rank_0(
            ...
            f"HD={c['head_dim'] or orig_head_dim}, "
...
def main(args):
    ...
    for i, candidate in enumerate(top_candidates):
        ...
        # head_dim is a post-prune slicing target, not an mcore_minitron
        # pruning target. prune_deepseek_v4.py reads it from args.head_dim
        # separately; pass it via the candidate dict for documentation.
        if candidate.get('head_dim'):
            export_config['head_dim'] = candidate['head_dim']
    ...
    if best_config.get('head_dim'):
        print_rank_0(f"  head_dim: {best_config['head_dim']}")
