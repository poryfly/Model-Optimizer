...
def main():
            ...
            for _lid in range(_n_layers):
                for _eid in range(_target_n_experts):
                    for _wn, _wr, _wc, _sr, _sc in [
                        ...
                        out_sd[f"layers.{_lid}.ffn.experts.{_eid}.{_wn}.weight"] = \
                            _nw[:_wr, :_wc].contiguous()
                        out_sd[f"layers.{_lid}.ffn.experts.{_eid}.{_wn}.scale"] = \
                            _ns[:_sr, :_sc].contiguous()
                        _mxfp4_count += 2

    if _mxfp4_count:
        print(f"  Sliced {_mxfp4_count // 2} expert weight pairs from native MXFP4")
    else:
    ...
    # Carry over quantization_config so SGLang uses FP8 DeepGEMM kernels
    # for the attention projections we just quantized above.
    # Set expert_dtype="fp4" since experts are stored as Marlin MXFP4.
    # Always set for V4 models — even if _fp8_count is 0, the model needs
    # the FP8 config so SGLang creates the right parameter types.
    if _fp8_count or any(
        k in out_sd for k in ("embed.weight", "head.weight")
    ):
        # Determine if shared_experts can use FP8 block quantization.
        # SGLang validates: output_partition_size % block_n == 0
        # where output_partition_size = moe_ffn / TP for shared experts.
        # block_n = 128 (DeepGEMM requirement).
        ...
        config["quantization_config"] = {
            ...
            "ignoredLayers": _ignored,
        }
    if _mxfp4_count or config.get("expert_dtype") == "fp4":
