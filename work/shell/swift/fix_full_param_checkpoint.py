...
def main():
    ...
    parser.add_argument(
        "--src",
        type=Path,
        default=Path("/data/wanghui/deepseekv4_train/megatron_input/dpsk-v4-4B-A1.5B"),
        help="源模型目录（含完整 config.json）",
    )
    parser.add_argument(
        "--dst",
        type=Path,
        default=Path("/data/wanghui/deepseekv4_train/megatron_output/dpsk-v4-4B-A1.5B_pt/v0-20260625-153154/checkpoint-25"),
    ...
    if expert_dtype == "bf16":
        ...
        # FP4 packed 权重：保留 expert_dtype = fp4
        if "quantization_config" in dst_config:
            del dst_config["quantization_config"]
            print("Removed quantization_config (weights are FP4 packed)")
